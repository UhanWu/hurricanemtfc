...
data_df <- TXhurprocss |>
  ...
  filter(sum == max(sum), .by = c(fips_code, date)) |>
...
reredf |>
  ...
  ggplot(mapping = aes(long, lat, group = group, fill = recover)) +
  ...
  labs(fill = "Resilience ", x = "Longitude", y = "Latitude")

centercounty %>% 
  mutate(county_fips = as.character(fips_code)) |>
  left_join(counties, by = "county_fips") %>% 
  filter(state_name =="Texas") %>% 
  ggplot(mapping = aes(long, lat, group = group, fill = distanceCoast)) +
  geom_polygon(color = "#ffffff", size = .25) +
  scale_fill_gradientn(colors = c("lightblue", "red"),
                       guide = guide_colorbar(title.position = "top")) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
...
plotfrequencyHUR %>% 
  ...
  labs(fill = "Avg Hit per year", x = "Longitude", y = "Latitude") + 
  theme_bw()
```
...
```{r}
reredf |>
  mutate(resilience = resilience/(60*60)) |>
  mutate(county_fips = as.character(fips_code)) |>
  left_join(counties, by = "county_fips") %>% 
  filter(state_name =="Texas") %>% 
  ggplot(mapping = aes(long, lat, group = group, fill = resilience)) +
  geom_polygon(color = "#ffffff", size = .25) +
  scale_fill_gradientn(colors = c("yellow", "purple"),
                       guide = guide_colorbar(title.position = "top")) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme(legend.title = element_text(),
        legend.key.width = unit(.5, "in")) +
...
reredf |>
  mutate(recover = recover/(60*60)) |>
  mutate(county_fips = as.character(fips_code)) |>
  left_join(s, by = "county_fips") %>% 
  filter(state_name =="Texas") %>% 
  ggplot(mapping = aes(long, lat, group = group, fill = recover)) +
  geom_polygon(color = "#ffffff", size = .25) +
  scale_fill_gradientn(colors = c("yellow", "purple"),
  ...
  theme(legend.title = element_text(),
        legend.key.width = unit(.5, "in")) +
  labs(fill = "Recovery ", x = "Longitude", y = "Latitude")


reredf |>
  select(fips_code) |>
merge(ULTIMATEdata, all = TRUE) |>
  select(fips_code, max_rateincrease, population) |>
  mutate(county_fips = as.character(fips_code)) |>
  left_join(counties, by = "county_fips") |>
  filter(state_name =="Texas") |>
  ggplot(mapping = aes(long, lat, group = group, fill = max_rateincrease/population)) +
  geom_polygon(color = "#ffffff", size = .25) +
  scale_fill_gradientn(colors = c("yellow", "purple"),
                       guide = guide_colorbar(title.position = "top")) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme(legend.title = element_text(),
        legend.key.width = unit(.5, "in")) +
  labs(fill = "Stability/Pop.", x = "Longitude", y = "Latitude")
