...
stormsfreq <- storm_winds |>
  ...
  summarise(frequency = n()/30, .by = fips) 

dffreqcoast <- plotfrequencyHUR |>
  merge(centercounty)
  
```

```{r}
modelfreq <- lm(dffreqcoast, formula = frequencyfinal ~ distanceCoast)
summary(modelfreq)
```
